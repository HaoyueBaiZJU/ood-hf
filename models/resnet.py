...
class SupCEHeadResNet(nn.Module):
    ...
    def __init__(self, args, multiplier = 1):
        ...
        #self.fc = nn.Linear(dim_in, 7)
        self.fc = nn.Linear(args.feat_dim, 7)
        ...
        self.nChannels = args.feat_dim
    
    def forward(self, x):

        feat = self.encoder(x).squeeze()
        unnorm_features = self.head(feat)
        #out= F.normalize(unnorm_features, dim=1)
        out = unnorm_features.view(-1, self.nChannels)
        return self.fc(out)
